...
const unsigned char D3_ToolTip::hash[11][8] = {
...
const unsigned char D3_ToolTip::parent_hash[11][8] = {
...
void D3_ToolTip::run()
{
    ...
    for(unsigned int i = min; i < max;)
    {
        ...
        if(s != 0 && (mbi.Type == MEM_PRIVATE) && (mbi.State == MEM_COMMIT))
        {
            ...
            if (! ReadProcessMemory(this->hndl, (LPVOID)mbi.BaseAddress, buffer, reg, NULL))
            {
                i += (unsigned int)mbi.RegionSize;
                continue;
            }
            for(unsigned int offset = 0; offset < reg - 0xD00; offset+=4)
            {
                for(unsigned char e = 0; e < ENUM_END; e++)
                {
                    ...
                    {
                        this->ptr[e] = (int)mbi.BaseAddress + offset + 0xA98;
                        this->ptr_count[e]++;
                        break;
                    }
                }
